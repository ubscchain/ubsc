...
echo 'alias ubsc="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/usbc" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/usbc'"
...
rm -rf "$HOME/.config/@ubsc"
rm -rf "$HOME/.config/@ubsc"
rm -rf "$HOME/.config/ubsc-core"
